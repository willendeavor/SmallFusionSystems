...
To load this fusion system we simply execute `SmallFusionSystem(3^5, i)`. If you want to find fusion systems based upon the isomorphism type of $S$ there is also functionality for this, `NumberSmallFusionSystems(S)` returns a list of the indices which correspond to fusion systems over $S$. The function `AllSmallFusionSystems(S)` will return a list of the actual fusion systems. A future update will most likely change the indexing so that fusion systems over the same group are grouped together and an index file maintained that keeps track of it all.

## Notes
All the fusion systems included were converted from those found in https://github.com/chris1961parker/Fusion-Systems as such there are some missing since `AllFusionSystems` runs assuming the fusion systems have trivial core and are $p$-perfect. It is possible to add a fusion system to the database via `AddSmallFusionSystem`, this relies on using `IsIsomorphic` to determine if the fusion system is already present and for the large fusion systems this becomes an arduous calculation. Once the format of the records have been improved and finalised (and therefore need rebuilding) I will most likely work on improving the execution time.


I have not done any extensive testing but initial observations seem to suggest that the loading of fusion systems in this package is faster than that of the FusionSystems package. For example running load "All3to6", which doing so returns a list of all fusion systems over groups of order $3^6$ took approx 150 seconds while `AllSmallFusionSystems(3^6)` took 75 seconds. As noted above saving is however fairly slow, adding all these fusion systems took 34 minutes.
